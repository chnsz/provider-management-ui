...
const renderApi = (v: any, row: Portal.OwnerSum, onClosed: () => any) => {
    const viewEle = renderCol((row.apiCount - row.apiNeedAnalysisCount || 0) / row.apiCount * 100, row.apiNeedAnalysisCount, row.apiCount, ['分析率', '待分析', '总数']);
    ...
    const expandedRowRender = (row) => {
        const columns: TableColumnsType<Portal.ProductSum> = [{
            ...
            render: (v, row) => {
                ...
                if (row.apiCount > 0) {
                    console.log(row)
                    rate = Math.round((row.apiCount - row.apiNeedAnalysisCount) / row.apiCount * 100);
                }
                const color = getColor(rate);
                const view = <div className={'column-cell'}>
                        <div className={'cell note'}>
